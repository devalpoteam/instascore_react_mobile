...
} from "@/services/api/users/userProfileService";
...
// Data and types
import { UserProfile, ProfileScreenState } from "../types/profile.types";
...
export default function ProfileScreen() {
  ...
  const loadProfileData = async () => {
    try {
      ...
      const profileData: UserProfile = {
        ...
        gender:
          apiProfileData.sexo === "Masculino"
            ? "masculino"
            : apiProfileData.sexo === "Femenino"
            ? "femenino"
            : "otro",
  ...
  if (!state.user && state.error) {
    return (
      <BaseLayout>
        <Header title="Perfil" subtitle="Error al cargar" showLogo={false} />
        <View
          style={{
            flex: 1,
            justifyContent: "center",
